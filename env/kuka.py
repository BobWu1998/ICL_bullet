...
class Kuka:
  ...
  def __init__(self, urdfRootPath=pybullet_data.getDataPath(), timeStep=0.01):
    ...
    self.gripper_closedness = False
    self.rollAngle = 0
    self.pitchAngle = -math.pi
    self.yawAngle = 0

    self.orn = p.getQuaternionFromEuler([self.rollAngle, 
                                    self.pitchAngle, 
                                    self.yawAngle])
  ...
  def applyAction(self, motorCommands):
    ...
    if (self.useInverseKinematics):
      ...
      if fingerAngle < 0.3 / 2:
        self.gripper_closedness = 1
      else:
        self.gripper_closedness = 0
      ...
      self.rollAngle += droll
      self.pitchAngle += dpitch
      self.yawAngle += dyaw
      ...
      # orn = p.getQuaternionFromEuler([0+math.pi/2,-math.pi,0])
      if (self.useNullSpace == 1):
        if (self.useOrientation == 1):
          jointPoses = p.calculateInverseKinematics(self.kukaUid, self.kukaEndEffectorIndex, pos,
                                                    self.orn, self.ll, self.ul, self.jr, self.rp)
        else:
